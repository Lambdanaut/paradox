...
func move_to(_new_x: int, _new_y: int, _colliding_piece=null) -> bool:
	...
	var pieces_at_pos = Globals.get_pieces_at(_new_x, _new_y)
	
	var can_move_piece: bool = true
	if not self in pieces_at_pos:
		for piece_at_pos in pieces_at_pos:
			...
			can_move_piece = can_move_piece and piece_at_pos.on_collided_with(
				colliding_piece, 
				recorded_movements[-1][0], 
				recorded_movements[-1][1])
